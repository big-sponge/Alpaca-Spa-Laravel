...
class Auth
{
    ...
    private static function getInstance()
    {
        if (!self::$instance) {
            ...
            //设置session.nam
            ini_set('session.name', 'HOME_MEMBER_ID');

            //设置session.save_path
            ini_set('session.save_path', RUNTIME_PATH . '/Session/Home');
    ...
    public function updateLoginInfo($loginType = null)
    {
        ...
        if ($_SESSION[self::LOGIN_MEMBER] && (empty($loginType) || $loginType = self::LOGIN_INFO_MEMBER)) {
            ...
            $data     = MemberModel::model(false)->getMemberInfo($memberId);
