...
class Gateway
{
     ...
     * 获取注册中心地址
     * @author Chengcheng
     * @date   2017年08月19日 20:39:25
     * @return array
     */
    public static function getRegisterAddress()
    {
        return static::$registerAddress;
    }
        ...
        if (self::$businessWorker) {
            ...
            if (!$all_addresses) {
                throw new Exception('Gateway::getAllGatewayAddressesFromRegister() with registerAddress:' .
                    static::getRegisterAddress() . '  return ' . var_export($all_addresses, true));
            }
    ...
    protected static function getAllGatewayAddressesFromRegister()
    {
        ...
        if(empty($addresses_cache) || $time_now - $last_update > $expiration_time) {
            $client = stream_socket_client('tcp://' . static::getRegisterAddress(), $errno, $errmsg, self::$connectTimeout);
            if (!$client) {
                throw new Exception('Can not connect to tcp://' . static::getRegisterAddress() . ' ' . $errmsg);
            }
            ...
            if (!$ret || !$data = json_decode(trim($ret), true)) {
                throw new Exception('getAllGatewayAddressesFromRegister fail. tcp://' .
                    static::getRegisterAddress() . ' return ' . var_export($ret, true));
            }
